...
import os
import numpy as np
from audio import grabar_audio, calcular_caract_Frecuencias, transcribir_audio_en_memoria, comparar_Frecuencias
from encriptar import doble_encriptacion_hill_aes, desencriptar_archivo, texto_a_matriz
...
#[Funcion para grabar un audio como clave, calcular su frecuencia y trasncribirlo a texto para convertir en una matriz]
def grabar_clave_encriptado():
    ...
    if audio is not None:
        ...
        huella_original= calcular_caract_Frecuencias(audio)
        texto= transcribir_audio_en_memoria("clave_encriptado.wav")
        matriz_original= texto_a_matriz(texto) if texto else None
...
#[Funcion para grabar un audio como clave de descencriptado, verifica si coinciden las frecuencias y desencriptan si son correctas]
def grabar_clave_desencriptado():
    ...
    if audio is not None:
        ...
        if comparar_Frecuencias(huella_original, huella_descifrado):
            texto = transcribir_audio_en_memoria("clave_desencriptado.wav")
            matriz_descifrado = texto_a_matriz(texto) if texto else None
...
#[Funcion para verificar que se haya seleccionado un archivo y grabado una clave de encriptado antes de proceder]  
...
#[Funcion para mostrar el mensaje de progreso]
...
#[Funcion para iniciar el proceso de encriptacion del archivo]
def encriptar():
...
#----------------------------------[COONFIGURACION INTERFAZ GRAFICA]----------------------------------#
root = tk.Tk()
...
root.mainloop() #iniciar interfaz
